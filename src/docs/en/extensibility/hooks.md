...
```text
matchesMatcher(matcher, subject)   — claude mode: literal-or-regex; codex: always regex
runHook(bash, hook, opts, now)     — stdin payload + env via ctx.shell; timeoutSec or 10-minute default
parseHookOutput(output)            — exit 2 blocks; others non-blocking
mergeHookOutputs(...)              — permission deny > ask > allow; halt is sticky; additionalContext accumulates
```
...
```yaml
# claude-code bridge (illustrative)
hooks-claude-code:
  config:
    configPath: .claude/settings.json   # existing hooks.json location
