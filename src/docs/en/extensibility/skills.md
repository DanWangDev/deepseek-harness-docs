...
```text
Disk (SKILL.md) → provider list() discovery → registry merges layered catalogs
  → the skill tool lists the catalog (filtered by modelInvocable)
  → the model calls skill("graphify")
  → the winning provider's get() returns a SkillDefinition
  → the body is injected via agent.inject() → reaches the request at the next pre-step
```
